    ...
    const tasks = rabRows
      ...
      .map((row) => {
        ...
        return {
          ...
          harga_upah,
          rab_key,
          desain: (row.get("desain") || "").toString() || matched?.desain || "",
          kualitas:
            (row.get("kualitas") || "").toString() || matched?.kualitas || "",
          spesifikasi:
            (row.get("spesifikasi") || "").toString() ||
            matched?.spesifikasi ||
            "",
...
app.post("/api/opname/item/submit", async (req, res) => {
  try {
    ...
    const rowValues = {
      ...
      desain: itemData.desain || "",
      kualitas: itemData.kualitas || "",
